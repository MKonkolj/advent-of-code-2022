...
let decryptedInput: string = gameInput;
...
const guidedRounds = rounds.map((round) => {
  if (round.me === "draw") {
    return { ...round, me: round.opponent };
  }
  if (round.me === "win") {
    let winningMoveIndex = moves.indexOf(round.opponent) + 1;
    return { ...round, me: moves[winningMoveIndex] };
  }
  if (round.me === "lose") {
    ...
    return { ...round, me: moves[losingMoveIndex] };
  }
});

const play = (opponent: Move, me: Move) => {
  if (opponent === me) {
    return 3;
  }
...
const scoreMatch = (rounds: any): number => {
  let score = 0;
...
const day2 = () => {
  messageTemplate(2, [
    `Once he explained what the second column means, my score was: ${scoreMatch(
      guidedRounds
    )}`,
  ]);
};
